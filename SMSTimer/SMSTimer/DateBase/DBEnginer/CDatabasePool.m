...
@interface CDatabasePool()

- (void)pushDatabaseBackInPool:(CDatabase*)db;
- (CDatabase*)db;

@end


@implementation CDatabasePool
...
- (id)initWithPath:(NSString*)aPath {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"CDB.%@", self] UTF8String], NULL);
        _databaseInPool     = CDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = CDBReturnRetained([NSMutableArray array]);
    }
...
- (void)dealloc {
    ...
    _delegate = 0x00;
    CDBRelease(_path);
    CDBRelease(_databaseInPool);
    CDBRelease(_databaseOutPool);
...
- (void)pushDatabaseBackInPool:(CDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The CDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
...
- (CDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if (db) {
            ...
            db = [CDatabase databaseWithPath:_path];
        }
