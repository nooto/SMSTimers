...
@interface CResultSet (Private)
- (NSMutableDictionary *)columnNameToIndexMap;
- (void)setColumnNameToIndexMap:(NSMutableDictionary *)value;
@end

@implementation CResultSet
@synthesize query=_query;
@synthesize columnNameToIndexMap=_columnNameToIndexMap;
@synthesize statement=_statement;

+ (id)resultSetWithStatement:(CStatement *)statement usingParentDatabase:(CDatabase*)aDB {
    
    CResultSet *rs = [[CResultSet alloc] init];
    
    [rs setStatement:statement];
    [rs setParentDB:aDB];
    
    return CDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    CDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return CDBReturnAutoreleased([dict copy]);
    }
    else {
...
//    if (sqlite3_column_type([_statement statement], columnIdx) == 5) {
//        NSLog(@"-----%d",sqlite3_column_type([_statement statement], columnIdx));
//    }
//    
...
- (void)setParentDB:(CDatabase *)newDb {
    _parentDB = newDb;
}
