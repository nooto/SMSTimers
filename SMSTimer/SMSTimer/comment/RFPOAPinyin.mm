...
{@"Zhang",@"张章长账仗丈掌涨账樟杖彰漳胀瘴障仉嫜幛鄣璋嶂獐蟑仧傽墇帳幥弡張慞扙承暲涱漲痮瘬瞕礃粀粻脹蔁賬遧鏱鐣長镸鞝餦騿鱆麞"},
...
{@"Zhu",@"住主猪竹株煮筑贮铸嘱拄注祝驻属术珠瞩蛛朱柱诸诛逐助烛蛀潴洙伫瘃翥茱苎橥舳杼箸炷侏铢疰渚褚躅麈邾槠竺予佇劅劚劯咮嗻噣囑坾墸壴孎宁宔尌屬嵀庶搊敱斀斗斸曯枓柠柷楮樦櫡櫧櫫欘殶泏泞澍濐瀦灟炢煑燭爥眝矚砫硃磩祩秼窋竚笁笜筯築篫篴紵紸絑纻罜羜芧苧茁茿莇蓫蕏薥藷藸蚰蝫蠋蠩蠾袾註詝誅諸豬貯跓跙跦軴迬逗逫鉏鉒銖鋳鑄钃阻除陼霔飳馵駐駯騶驺鮢鯺鱁鴸鸀麆鼄著丶"},
{@"Zhua",@"抓爪挝摣撾檛簻膼髽"},
{@"Zhuai",@"拽转尵捙睉跩顡嘬"},
...
+ (NSString *)quickConvert:(NSString *)hzString {
    ...
    return [RFChineseToPinyin pinyinFromChiniseString:hzString];
    ...
    if (cache == NULL) {
        ...
        cache = (short *)malloc(cacheLength * sizeof(pinyinTableCell *));
...
{
    if (str >= HANZI_START && str <= HANZI_START+HANZI_COUNT)
        return YES;
    return NO;
}
...
{
    NSString * tempText=[NSString stringWithCharacters:&hanzi length:1];
    return [RFChineseToPinyin sortSectionTitle:tempText];
}
