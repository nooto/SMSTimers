...
@class CGDataXMLElement,CGDataXMLDocument;
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *localName = [CGDataXMLNode localNameForName:name];
...
@interface CGDataXMLNode (PrivateMethods)
...
- (CGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
+ (CGDataXMLElement *)elementWithName:(NSString *)name {
...
+ (CGDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
...
+ (CGDataXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  if (theXMLNode->type == XML_ELEMENT_NODE) {
    theClass = [CGDataXMLElement class];
  } else {
    theClass = [CGDataXMLNode class];
  }
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
     CGDataXMLNode *node = [CGDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (CGDataXMLNode *)childAtIndex:(unsigned)index {
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
           ...
           CGDataXMLNode *node = [CGDataXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
...
- (BOOL)isEqual:(CGDataXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[CGDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (__bridge void *) [CGDataXMLNode class];
}
...
@implementation CGDataXMLElement
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
       CGDataXMLNode *node = [CGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)addNamespace:(CGDataXMLNode *)aNamespace {
...
- (void)addChild:(CGDataXMLNode *)child {
...
- (void)removeChild:(CGDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    for (CGDataXMLNode *child in children) {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
     ...
     CGDataXMLNode *node = [CGDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(CGDataXMLNode *)attribute {
...
- (CGDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  ...
  for (CGDataXMLNode *attr in attributes) {
...
- (CGDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
     CGDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (CGDataXMLNode *)attributeForLocalName:(NSString *)localName
...
@interface CGDataXMLDocument (PrivateMethods)
- (void)addStringsCacheToDoc;
@end

@implementation CGDataXMLDocument

- (id)initWithXMLString:(NSString *)str options:(unsigned int)mask error:(NSError **)error {

  NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
 CGDataXMLDocument *doc = [self initWithData:data options:mask error:error];
  return doc;
}
...
- (id)initWithRootElement:(CGDataXMLElement *)element {
...
- (CGDataXMLElement *)rootElement {
CGDataXMLElement *element = nil;

  if (xmlDoc_ != NULL) {
    xmlNodePtr rootNode = xmlDocGetRootElement(xmlDoc_);
    if (rootNode) {
      element = [CGDataXMLElement nodeBorrowingXMLNode:rootNode];
    }
  }
  return element;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
   CGDataXMLNode *docNode = [CGDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
